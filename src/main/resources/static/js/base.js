...
import {userTemplate, reservationTemplate} from './template/UserTemplate.js';

$('.logout') && $('.logout').addEventListener('click', logout);
$('.user') && $('.user').addEventListener('click', user);
...
function user(e) {
    e.preventDefault();
    const userId = $('.user').dataset.column;
    fetch('/api/users/' + userId)
        .then(response => {
            if (!response.ok) {
                throw '회원 상세 불러오기에 실패하였습니다.';
            }
